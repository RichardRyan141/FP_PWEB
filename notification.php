            ...
            <tbody id="data-table-body">
                <?php
                    ...
                    if ($userType == "Pemilik") {
                        $sql = "SELECT *
                                FROM transaksi
                                WHERE StatusTransaksi_st_id != 3 AND DATE(t_waktu) = CURDATE()
                                ORDER BY t_waktu";
                    } else {
                        $locationQuery = "SELECT l_id
                                          FROM lokasi l
                                          JOIN karyawan k ON k.Lokasi_l_id = l.l_id
                                          WHERE k.k_nama = '$username'";
                        $locationResult = mysqli_query($db, $locationQuery);
                        $locationRow = mysqli_fetch_assoc($locationResult);
                        $locID = $locationRow['l_id'];
                        $sql = "SELECT *
                                FROM transaksi
                                WHERE StatusTransaksi_st_id != 3 AND DATE(t_waktu) = CURDATE() AND Lokasi_l_id = '$locID'
                                ORDER BY t_waktu";
                    }
